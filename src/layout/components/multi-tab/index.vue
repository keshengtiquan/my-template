...
const changeActive = (fullPath: string) => {
  multiTabStore.switchTab(fullPath)
}
...
<template>
  <div v-if="layoutSetting.multiTabFixed" :style="{ height: `${48}px` }" />
  <!-- <a-tabs
      ref="tabsRef"
...
<template #rightExtra>
  ...
  <div :style="tabStyle" class="cursor-pointer w-full bg-[var(--bg-color)] h-40px flex items-center justify-between">
    <div flex items-center>
      <div class="tab-item" :class="{ 'active': item.fullPath === activeKey }" v-for="item in list" :key="item.fullPath"
        @click="changeActive(item.fullPath)">
        {{ item.title }}
        <CloseOutlined class="font-size-12px  hover:bg-blue b-rd-6px hover:text-light"
          v-if="!item.affix && list.length > 1" @click.stop="multiTabStore.close(item.fullPath)" />
      </div>
    </div>
    <div flex items-center>
      <div w-40px h40px flex justify-center border-l border-l-solid border-warmgray-2>
        <ReloadOutlined />
      </div>
      <div w-40px h40px flex justify-center border-l border-l-solid border-warmgray-2>
        <DownOutlined flex justify-center />
      </div>
    </div>
