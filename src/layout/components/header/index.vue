...
const {changeToCompany, changeToProject} = useChangeRouter()
...
<template>
  <a-layout-header :style="headerStyle">
    ...
    <div class="flex items-center">
      <div @click="toCompany" class="p-x-12px cursor-pointer hover:text-blue" :class="[userStore.moduleType === 'company' ? 'text-blue' : ''] ">
        <HomeOutlined /><span ml-10px>企业工作台</span>
      </div>
      <div @click="toProject" class="p-x-12px cursor-pointer  hover:text-blue" :class="[userStore.moduleType === 'project' ? 'text-blue' : ''] ">
        <DesktopOutlined /><span ml-10px>项目工作台</span>
      </div>
    </div>
    <a-breadcrumb flex items-center v-if="!route.meta.hideInBreadcrumb && layoutSetting.layout === 'side'">
    ...
    <UserAvatar />
    <Setting v-model:open="layoutSetting.drawerVisible" :theme="layoutSetting.theme"
      :colorPrimary="layoutSetting.colorPrimary" :layout="layoutSetting.layout"
      :contentWidth="layoutSetting.contentWidth" :fixedHeader="layoutSetting.fixedHeader"
      :splitMenus="layoutSetting.splitMenus" :header="layoutSetting.header" :footer="layoutSetting.footer"
      :menu="layoutSetting.menu" :colorWeak="layoutSetting.colorWeak" :multiTab="layoutSetting.multiTab"
      :multiTabFixed="layoutSetting.multiTabFixed" :keepAlive="layoutSetting.keepAlive"
      :leftCollapsed="layoutSetting.leftCollapsed" :showLogo="layoutSetting.showLogo"
      :animation-name-list="animationNameList" :animation-name="layoutSetting.animationName"
      :accordionMode="layoutSetting.accordionMode" @setting-change="appStore.changeSettingLayout"></Setting>
  </a-layout-header>
</template>
