...
<template>
  <div>
    <pro-table bordered v-if="columns.length > 7"
      @search="(params) => getTableData({ ...params, ...{ sectionalEntry: activeKey } })" :loading="loading"
      @refresh="() => getTableData({ sectionalEntry: activeKey })" :pagination="pagination" :data-source="tableData"
      :scroll="{ x: 1500, y: 455 }" :columns="columns" table-size="small">
      <template #toolLeft>
      ...
      <template #toolRight>
        <a-space>
          <a-button v-if="hasAccess(['sys:workplace:uploadCollect'])" type="primary"
            @click="() => open = true">导入数据</a-button>
          <ExportButton v-if="hasAccess(['sys:workplace:exportCollect'])"
            :request="exportWorkPlaceRelevanceCollectListApi" :init-params="{ sectionalEntry: activeKey }"
            file-name="车站区间工程量汇总表"></ExportButton>
        </a-space>
      </template>
