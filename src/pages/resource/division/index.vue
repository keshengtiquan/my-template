...
<template>
  <PageContainer>
    <ProTable :columns="columns" :defaultExpandedRowKeys="expandedRowKeys" ref="divisionTableRef" rowKey="id"
      :data-source="tableData" :loading="loading" @refresh="() => getTableData()" :pagination="false">
      <template #toolLeft>
        <a-space>
          <a-button v-if="hasAccess(['sys:division:add'])" type="primary" @click="createDivision">新增</a-button>
          <a-button v-if="hasAccess(['sys:division:upload'])" @click="() => open = true">导入数据</a-button>
        </a-space>
      </template>
      <template #bodyCell="{ column, record }">
        <template v-if="column.dataIndex === 'actions'">
          <a-space :size="20">
            <TooltipIcon v-if="hasAccess(['sys:division:edit'])" title="编辑">
            ...
            <TooltipIcon v-if="hasAccess(['sys:division:addSub'])" title="添加下级">
              <PlusOutlined class="text-blue" @click="addSub(record)" />
            </TooltipIcon>
            <TooltipIcon v-if="hasAccess(['sys:division:delete'])" title="删除">
              <DeleteOutlined class="text-red" @click="deleteDivision(record)" />
            </TooltipIcon>
          </a-space>
        </template>
        <template v-else-if="column.dataIndex === 'divisionName'">
          <a-typography-paragraph @click="viewProjectList(record)">
            <a>{{ record.divisionName }}</a>
          </a-typography-paragraph>
        </template>
      </template>
    </ProTable>
    <Upload v-model:open="open" width="70%" :is-multiple="false" :request="uploadDivisionApi" :upload-type="['xlsx']"
      service-name="DivisionImport"></Upload>
