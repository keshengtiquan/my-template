...
import { loginApi } from "@/api/setting/user";
...
const submit = async () => {
  ...
  try {
    ...
    if (data.token) {
      ...
      const type = userStore.moduleType
      console.log(39);
      
      // 非系统用户直接跳转项目工作台
      // 系统用户根据当前的模式进行跳转
      if (userType && userType !== UserTypeEnum.SYSTEM_USER) {
        ...
        <div class="ant-pro-form-login-main" w-335px>
          <a-form ref="formRef" :model="loginModel">
            <a-form-item name="userName" :rules="[{ required: true, message: '用户名不能为空' }]">
              <a-input v-model:value="loginModel.userName" allow-clear placeholder="用户名：admin or user" size="large"
                @pressEnter="submit">
                <template #prefix>
                  <UserOutlined />
                </template>
            ...
            <a-form-item name="password" :rules="[{ required: true, message: '密码不能为空' }]">
              <a-input-password v-model:value="loginModel.password" allow-clear placeholder="密码：admin" size="large"
                @pressEnter="submit">
