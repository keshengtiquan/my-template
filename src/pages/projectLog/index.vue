...
import {getProjectLogListApi} from "@/api/projectLog";
...
const columns = ref<any[]>([
  {
    ...
    align: 'center',
    width: 50,
    customRender: ({text, record, index, column}) => `${(pagination.value.current - 1) * pagination.value.pageSize + parseInt(index) + 1 }`,
  },
...
<template>
  <PageContainer>
    <pro-table v-if="columns.length > 9" :columns="columns" :data-source="tableData" :loading="loading"
               ...
               :scroll="{x: 1500}" @refresh="() => getTableData()" @search="(params) => getTableData(params)">
