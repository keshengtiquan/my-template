...
import {createGanttApi} from "@/api/gantt";
...
const showModal = (id: string) => {
  formState.parent = id || '0';
  open.value = true;
};

const handleOk = async () => {
  await formRef1.value.validate()
  const res = await createGanttApi(formState)
  if (res.code === 200) {
...
onMounted(() => {
  formState.endDate = calcEndDate(dayjs(new Date()).format('YYYY-MM-DD'), formState.duration);
})
